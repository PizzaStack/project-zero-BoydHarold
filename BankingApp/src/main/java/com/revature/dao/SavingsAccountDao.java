...
public class SavingsAccountDao {
	...
	public List<SavingsAccount> getAllSavingsAccounts() {
		...
		try {
			...
			while (rs.next()) {
				savingsAccount = new SavingsAccount(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getString(4),
						rs.getDouble(5));
	...
	public SavingsAccount getSavingsAccountById(int customerId) {
		...
		try {
			...
			preGetSavingsAccount = EstablishConnection.connection
					.prepareStatement("SELECT * FROM SavingsAccount WHERE CustomerId = ?");
	...
	public void updateSavingsAccount(SavingsAccount savingsAccount) {
		try {
			PreparedStatement preUpdateSavingsAccount = EstablishConnection.connection.prepareStatement(
					"UPDATE SavingsAccount SET Status = ?, ApprovalStatus = ?, Balance = ? WHERE CustomerId = ?;");
	...
	public void addSavingsAccount(SavingsAccount savingsAccount) {
		try {
			PreparedStatement preAddSavingsAccount = EstablishConnection.connection.prepareStatement(
					"INSERT INTO SavingsAccount (CustomerId, Status, ApprovalStatus, Balance) VALUES (?,?,?,?);");
