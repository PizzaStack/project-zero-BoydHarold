...
public class AdministrativeFunctionsDao {
	...
	public void cancelAccount(int id, String accountType) {
		...
		if (accountType.equals("1")) {
			...
			try {
				PreparedStatement preCancelCustomerAccount = EstablishConnection.connection
						.prepareCall("SELECT cancelcustomeraccount(?);");
			...
			try {
				PreparedStatement preCancelAdministratorAccount = EstablishConnection.connection
						.prepareCall("SELECT canceladminaccount(?);");
	...
	public void activateAccount(int id, String accountType) {
		if (accountType.equals("1")) {
			try {
				PreparedStatement preCancelCustomerAccount = EstablishConnection.connection
						.prepareCall("SELECT activatecustomeraccount(?);");
				preCancelCustomerAccount.setInt(1, id);
			...
			try {
				PreparedStatement preCancelEmployeeAccount = EstablishConnection.connection
						.prepareCall("SELECT activateemployeeaccount(?);");
				preCancelEmployeeAccount.setInt(1, id);
				preCancelEmployeeAccount.executeUpdate();
			} catch (SQLException e) {
			...
			try {
				PreparedStatement preCancelAdministratorAccount = EstablishConnection.connection
						.prepareCall("SELECT activateadminaccount(?);");
				preCancelAdministratorAccount.setInt(1, id);
				preCancelAdministratorAccount.executeUpdate();
			} catch (SQLException e) {
