...
public class CheckingAccountDao {
	...
	public List<CheckingAccount> getAllCheckingAccounts() {
		...
		try {
			...
			preGetAllCheckingAccounts = EstablishConnection.connection
					.prepareStatement("SELECT * FROM CheckingAccount;");
			ResultSet rs = preGetAllCheckingAccounts.executeQuery();
			CheckingAccount checkingAccount = null;
			while (rs.next()) {
				checkingAccount = new CheckingAccount(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getString(4),
						rs.getDouble(5));
	...
	public CheckingAccount getCheckingAccountById(int customerId) {
		...
		try {
			...
			preGetCheckingAccount = EstablishConnection.connection
					.prepareStatement("SELECT * FROM CheckingAccount WHERE CustomerId = ?");
	...
	public void updateCheckingAccount(CheckingAccount checkingAccount) {
		try {
			PreparedStatement preUpdateCheckingAccount = EstablishConnection.connection.prepareStatement(
					"UPDATE CheckingAccount SET Status = ?, ApprovalStatus = ?, Balance = ? WHERE CustomerId = ?;");
	...
	public void addCheckingAccount(CheckingAccount checkingAccount) {
		try {
			PreparedStatement preAddCheckingAccount = EstablishConnection.connection.prepareStatement(
					"INSERT INTO CheckingAccount (CustomerId, Status, ApprovalStatus, Balance) VALUES (?,?,?,?);");
